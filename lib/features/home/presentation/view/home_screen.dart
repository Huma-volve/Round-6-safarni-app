...
class HomeScreen extends StatelessWidget {
  ...
  Widget circularProgressWidget() {
    return const Center(
      child: CircularProgressIndicator.adaptive(
        backgroundColor: AppColors.viewAllColor,
      ),
    );
  }
  ...
  Widget buildRecommendationWidget() {
    return BlocBuilder<RecommendedToursCubit, RecommendedToursState>(
      buildWhen: (previous, current) =>
          current is RecommendedToursLoading ||
          current is RecommendedToursLoaded ||
          current is RecommendedToursError,
      builder: (context, state) {
        if (state is RecommendedToursLoading ||
            state is RecommendedToursInitial) {
          return circularProgressWidget();
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return AvailableTourItem(
                    availableTourModel: state.availableTours[index],
                  );
                },
              );
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: MultiBlocProvider(
        providers: [
          BlocProvider(
            create: (_) => RecommendedToursCubit()
              ..getRecommendedTours()
              ..getCategories(),
          ),
          BlocProvider(
            create: (_) => AvailableToursCubit()..getAllAvailableTours(),
          ),
        ],
        child: Scaffold(
